...
func NewRouter(h *handler.Handlers, m *middleware.Middlewares, telegramWebhook http.Handler) http.Handler {
	...
	mux.Handle("PATCH /v1/boards/{boardId}", protected(h.Boards.Update))
	mux.Handle("DELETE /v1/boards/{boardId}", protected(h.Boards.Delete))
	mux.Handle("GET /v1/boards", protected(h.Boards.List))
	...
	mux.Handle("PATCH /v1/boards/{boardId}/columns/{columnId}", protected(h.Columns.Update))
	...
	mux.Handle("PATCH /v1/boards/{boardId}/columns/{columnId}/tasks/{taskId}", protected(h.Tasks.Update))
